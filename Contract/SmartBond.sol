...
contract SmartBond {
    ...
    uint256 public constant MATURITY_PERIOD = 3 * 365 days;
    uint256 public constant PAYMENT_FREQUENCY = 1 * 365 days;
    uint256 public bondBoughtDate;
    bool public bondFullySubscribed = false;

    address[] public bondholderAddresses;
    ...
    function buyBond() external payable notFullySubscribed {
        ...
        bondholderAddresses.push(msg.sender);
    ...
    function payCoupon() external onlyIssuer {
        ...
        for (uint256 i = 0; i < bondholderAddresses.length; i++) {
            address bondholder = bondholderAddresses[i];
